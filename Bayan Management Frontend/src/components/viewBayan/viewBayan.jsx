// src/components/viewBayan/viewBayan.jsx
...
const Viewbayan = () => {
    ...
    useEffect(() => {
        ...
        const fetchBayans = async () => {
            try {
                setLoading(true);
                console.log("Fetching with token:", axios.defaults.headers.common["Authorization"]);
                ...
                const flattenedData = response.data.flatMap((doc) =>
                    doc.bayans.map((bayan) => ({
                        ...bayan,
                        docId: doc._id,
                        serialNumber: bayan.serialNumber || serialNumberCounter++,
                    }))
                );
                ...
                setError(error.response?.data?.message || "Failed to load bayans");
    ...
    const handleAddBayan = async () => {
        try {
            ...
            setFormattedMessage(`${formatDate(fromDate)} முதல் ${formatDate(toDate)} வரை பயான்கள்`);
            const newBayan = {
            ...
            console.log("Adding bayan with data:", bayanData);
            ...
            console.error("Error adding bayan:", error.response?.data || error.message);
            setError(error.response?.data?.message || "Failed to add bayan");
            setIsErrorModalOpen(true);
            if (error.response?.status === 401) {
                logout();
                navigate("/");
    ...
    const handleClearBayans = async () => {
        try {
            ...
            console.error("Error clearing bayans:", error.response?.data || error.message);
            setError(error.response?.data?.message || "Failed to clear bayans");
            setIsErrorModalOpen(true);
            if (error.response?.status === 401) {
                logout();
                navigate("/");
    ...
    const handleSaveEdit = async () => {
        if (selectedBayan && selectedBayan._id && selectedBayan.docId) {
            try {
                ...
                console.error("Error updating bayan:", error.response?.data || error.message);
                setError(error.response?.data?.message || "Failed to update bayan");
                setIsErrorModalOpen(true);
                if (error.response?.status === 401) {
                    logout();
                    navigate("/");
    ...
    const confirmCancel = async () => {
        if (selectedBayan && selectedBayan._id && selectedBayan.docId) {
            try {
                ...
                console.error("Error deleting bayan:", error.response?.data || error.message);
                setError(error.response?.data?.message || "Failed to delete bayan");
                setIsErrorModalOpen(true);
                if (error.response?.status === 401) {
                    logout();
                    navigate("/");
    ...
    if (isLoading || loading) return <div className="loading">Loading <span className="spinner">...</span></div>;
                ...
                <div className="cancel-bayan-modal">
                    <div className="overlay">
                        <div className="modal-content">
                            ...
                            <p><strong>நாள்:</strong> {selectedBayan.dayOfTheWeek || "N/A"}</p>
                            <p><strong>நேரம்:</strong> {selectedBayan.numericTime || "N/A"}</p>
                            <p><strong>இடம்:</strong> {selectedBayan.place || "N/A"}</p>
                            <p><strong>பேச்சாளர்:</strong> {selectedBayan.speaker || "N/A"}</p>
                    ...
                    <div className="overlay">
                        <div className="modal-content">
                            ...
                            <div className="add-bayan-container">
                                <p><strong>தேதி வரம்பு (Date Range):</strong></p>
                                ...
                                <p id="time-msg"><strong>நேரம்:</strong></p>
                                ...
                                <p id="place-msg"><strong>இடம்:</strong></p>
                                <div className="dropdown">
                                    <button
                                        className="btn btn-secondary dropdown-toggle"
                                        type="button"
                                ...
                                <p id="speaker-msg"><strong>பேச்சாளர்:</strong></p>
                                <div className="dropdown">
                                    <button
                                        className="btn btn-secondary dropdown-toggle"
                                        type="button"
